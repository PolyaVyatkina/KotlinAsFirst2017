...
fun isLatinSquare(matrix: Matrix<Int>): Boolean {
    ...
    val n = matrix.height
    for (i in 1..n) checkList.add(i)
    for (i in 0 until n) {
        val midList1 = mutableListOf<Int>()
        val midList2 = mutableListOf<Int>()
        for (j in 0 until n) {
            midList1.add(matrix[i, j])
            midList2.add(matrix[j, i])
        }
        if (midList1.sorted() != checkList || midList2.sorted() != checkList) return false
    }
    return true
}
